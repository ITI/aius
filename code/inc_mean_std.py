...
class ExpMeanSTD():
    def __init__(self, norm, alpha, delta=0):
        self.norm = norm 
        self.total = 0
        self.alpha = alpha
        self.delta = delta
    ...
    def detect(self, x):
        if self.total >= 2 and abs(x-self.mean) > max(self.delta, self.getSTD()):
            if x > self.mean:
                rst = 1
            else:
                rst = -1
