...
event dnp3_response_data_object(c: connection, is_orig: bool, data_value: count)
{
    ...
    if(dnp3_group == 1)
    {
        ...
        while(i < 8 && dnp3_shift < g_quantity) {
            event DataLevel::data_get([$ts=ts, $conn=c, $protocol=g_pro, $uid=g_uid, $data_type="Binary", $index=g_start+dnp3_shift, $value=(data_value%2==1)?1.0:-1.0]);
    ...
    else if(dnp3_group == 2)
    {
        i = 0;
        while(i < 8 && dnp3_shift < g_quantity) {
            event DataLevel::data_get([$ts=ts, $conn=c, $protocol=g_pro, $uid=g_uid, $data_type="Binary", $index=g_start+dnp3_shift, $value=(data_value%2==1)?1.0:-1.0, $is_event=T]);
            #print(cat("Binary ", g_start+dnp3_shift, ":", data_value%2));
            data_value = data_value / 2;
            ++dnp3_shift;
            ++i; 
...
{
    local ts: time = network_time();
    event DataLevel::data_get([$ts=ts, $conn=c, $protocol=g_pro, $uid=g_uid, $data_type="Analog", $index=g_start+dnp3_shift, $value=int32_convert(value)]);
...
{
    local ts: time = network_time();
    event DataLevel::data_get([$ts=ts, $conn=c, $protocol=g_pro, $uid=g_uid, $data_type="Analog", $index=g_start+dnp3_shift, $value=int32_convert(value), $is_event=T]);
    #print(cat("Int32 ", g_start+dnp3_shift, ":", int32_convert(value)));
    ++dnp3_shift;
    if(dnp3_shift == g_quantity) {
        dnp3_shift = 0;
...
{
    local ts: time = network_time();
    event DataLevel::data_get([$ts=ts, $conn=c, $protocol=g_pro, $uid=g_uid, $data_type="Analog", $index=g_start+dnp3_shift, $value=int16_convert(value)]);
...
{
    local ts: time = network_time();
    event DataLevel::data_get([$ts=ts, $conn=c, $protocol=g_pro, $uid=g_uid, $data_type="Analog", $index=g_start+dnp3_shift, $value=int16_convert(value), $is_event=T]);
    #print(cat("Int16 ", g_start+dnp3_shift, ":", int16_convert(value)));
    ++dnp3_shift;
    if(dnp3_shift == g_quantity) {
        dnp3_shift = 0;
...
{
    local ts: time = network_time();
    event DataLevel::data_get([$ts=ts, $conn=c, $protocol=g_pro, $uid=g_uid, $data_type="Analog", $index=g_start+dnp3_shift, $value=float_convert(value)]);
...
{
    local ts: time = network_time();
    event DataLevel::data_get([$ts=ts, $conn=c, $protocol=g_pro, $uid=g_uid, $data_type="Analog", $index=g_start+dnp3_shift, $value=float_convert(value), $is_event=T]);
    #print(cat("Float ", g_start+dnp3_shift, ":", float_convert(value)));
    ++dnp3_shift;
    if(dnp3_shift == g_quantity) {
        dnp3_shift = 0;
...
{
    local ts: time = network_time();
    event DataLevel::data_get([$ts=ts, $conn=c, $protocol=g_pro, $uid=g_uid, $data_type="Analog", $index=g_start+dnp3_shift, $value=double_convert(value_low, value_high)]);
...
{
    local ts: time = network_time();
    event DataLevel::data_get([$ts=ts, $conn=c, $protocol=g_pro, $uid=g_uid, $data_type="Analog", $index=g_start+dnp3_shift, $value=double_convert(value_low, value_high), $is_event=T]);
    #print(cat("Double ", g_start+dnp3_shift, ":", double_convert(value_low, value_high)));
    ++dnp3_shift;
    if(dnp3_shift == g_quantity) {
        dnp3_shift = 0;
...
{
    local ts: time = network_time();
    event DataLevel::data_get([$ts=ts, $conn=c, $protocol=g_pro, $uid=g_uid, $data_type="Counter", $index=g_start+dnp3_shift, $value=int32_convert(count_value)]);
...
{
    local ts: time = network_time();
    event DataLevel::data_get([$ts=ts, $conn=c, $protocol=g_pro, $uid=g_uid, $data_type="Counter", $index=g_start+dnp3_shift, $value=int16_convert(count_value)]);
