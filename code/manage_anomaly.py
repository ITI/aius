...
class AnomalyManager():
    ...
    def updateMetaAlert(self, anomaly, meta_alert):
        if meta_alert["desp"] == "NEW_ORIG" or meta_alert["desp"] == "NEW_RESP":
            meta_alert["index"] = anomaly.aggregateIndex(meta_alert["index"])
    ...
    def aggregate(self, anomaly):
        ...
        desp = anomaly.getDesp()
        if desp == "NEW_ORIG" or desp == "NEW_RESP":
            candidates = meta_alerts.find({"desp": desp})      
            for candidate in candidates:
                if anomaly.matchIndex(candidate["index"]) and (
                    anomaly.getTS() > candidate["ts"][0] - self.max_time_gap or
    ...
    def isCriticalOperation(self, meta_alert):
        ...
        if (meta_alert["anomaly_type"] == "packet" or
            ...
            meta_alert["anomaly_type"] == "measurement" or
            meta_alert["operation"] is None):
            return unknown 
        else:
